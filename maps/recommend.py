...
def find_centroid(cluster):
    ...
    for i in range(len(cluster)):
        latitudes += [restaurant_location(cluster[i])[0]]
        longitudes += [restaurant_location(cluster[i])[1]]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for i in range(len(feature_fns)):
        predictor, r_squared = find_predictor(user, reviewed, feature_fns[i])
...
def rate_all(user, restaurants, feature_fns):
    ...
    for i in range(len(restaurants)):
        ...
        if restaurants[i] in reviewed:
            all_ratings[name] = user_rating(user, restaurant_name(reviewed[i]))
        else:
            all_ratings[name] = predictor(restaurants[i])
...
def search(query, restaurants):
    ...
    for i in range(len(restaurants)):
        if query in restaurant_categories(restaurants[i]):
            suitable_restaurants.append(restaurants[i])
